...
export default function Programmers() {
  ...
  const [programmerId, setId] = useState(null);

  const getNameOfFeatured = (programmer, id) => {
  ...
  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: programmerId ? "red" : "royalblue", // 🤔
  };
    ...
    <div className="widget-programmers container">
      ...
      <div className="programmers">
        {
          ...
          programmers.map((dev) => (
            <div key={dev.id}>
              ...
              <button
                onClick={() => {
                  /* set the featured id passing dev.id */
                  setId(dev.id);
                }}
              >
                Feature
      ...
      </div>
      {
        ...
        programmerId !== null ? (
          <div style={style}>
            🎉 Let&apos;s celebrate{" "}
            {getNameOfFeatured(programmers, programmerId)}! 🥳
          </div>
        ) : (
